...
- **自动化可控**：可启动进程级文件监听并去抖增量索引，但绝不自动接受长期记忆候选。
- **备份可加密**：支持 scrypt + AES-256-GCM 的流式加密备份，口令不写入数据库或参数。
- **本地项目工作台**：通过仅监听本机的 Web UI 查看项目画像、编辑五级个人规则，并预览真实上下文组装结果。
...
| 视图 | 能力 |
| --- | --- |
| 项目画像 | 汇总项目身份、索引状态、代码规模、文件类型、Git 状态、记忆、候选、任务和主要来源；关系图支持拖拽、缩放、筛选、搜索、布局切换和按需展开符号邻居 |
...
`project_watch_start` 创建的 watcher 只存在于当前 MCP/CLI 进程内，不写入 SQLite。它把密集文件事件按 `debounceMs` 合并，然后调用与手动命令完全相同的增量 `project_index`。`project_watch_list` 会报告是否待执行、是否正在索引、最近事件、最近成功索引和最近错误；`project_watch_stop` 立即停止监听。

进程重启后 watcher 不会自动恢复，CLI 的 `watch` 命令需要保持运行。大型项目可以不启用 watcher，继续在会话开始或任务结束时手动索引。无论手动还是自动索引，新发现的长期信息都只会成为 `pending` 候选；watcher 永远不会调用 `memory_candidate_accept`。
